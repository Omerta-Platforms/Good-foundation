...
-- NOTE: The default admin row that used to live here has been removed.
-- Admin access is now gated by a shared dashboard password (see
-- ADMIN_DASHBOARD_PASSWORD in .env.local) rather than a database row,
-- so no admin seed row is needed. See database-rls-policies.sql for
-- the required follow-up RLS setup step.
